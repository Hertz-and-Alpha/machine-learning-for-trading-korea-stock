...
def load_equities():
    return pd.read_hdf(custom_data_path / 'fnguide.h5', 'kr/equities')

def data_generator():
    for sid, symbol, asset_name in ticker_generator():
        df = pd.read_hdf(custom_data_path / 'fnguide.h5', 'kr/{}'.format(sid))
...
def fnguide_kr_to_bundle(interval='1d'):
    ...
    def ingest(environ,
        ...
        # empty DataFrame
        adjustment_writer.write(splits=pd.read_hdf(custom_data_path / 'fnguide.h5', 'kr/splits'))

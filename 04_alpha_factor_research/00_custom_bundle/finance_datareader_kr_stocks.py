...
def load_equities():
    return pd.read_hdf(custom_data_path / 'finance_datareader.h5', 'kr/equities')

def data_generator():
    for sid, symbol, asset_name in ticker_generator():
        df = pd.read_hdf(custom_data_path / 'finance_datareader.h5', 'kr/{}'.format(sid))
...
def finance_datareader_kr_to_bundle(interval='1d'):
    def ingest(environ,
               asset_db_writer,
               minute_bar_writer,
               daily_bar_writer,
        ...
        # empty DataFrame
        adjustment_writer.write(splits=pd.read_hdf(custom_data_path / 'finance_datareader.h5', 'kr/splits'))
